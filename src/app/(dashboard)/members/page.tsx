  ...
  return (
    <div className="min-h-screen bg-white px-4 py-6 lg:px-8 lg:py-12">
      <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-3 space-y-2">
              {myMembers.map((m) => (
                <button
                  key={m.id}
                  onClick={() => router.push(`/members/${m.id}`)}
                  className="flex w-full items-center gap-3 rounded-[4px] border border-brand-gray bg-white p-3 text-left shadow-sm transition hover:border-primary"
                >
        ...
        {!query && isAdmin && allMembers.length > myMembers.length && (
          <div className="mt-6">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-primary/40">All Team Members</h2>
            <div className="mt-3 space-y-2">
              {allMembers
                .filter((m) => !myMembers.some((my) => my.id === m.id))
                .map((m) => (
                  <button
                    key={m.id}
                    onClick={() => router.push(`/members/${m.id}`)}
                    className="flex w-full items-center gap-3 rounded-[4px] border border-brand-gray/50 bg-white p-3 text-left shadow-sm transition hover:border-primary"
                  >
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-primary">{m.name}</p>
                      {m.role && <p className="text-xs text-primary/50">{m.role}</p>}
